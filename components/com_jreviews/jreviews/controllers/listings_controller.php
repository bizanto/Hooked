...
/*                ,'joins'=>array(  # They are added in the Everywhere component when the listing is queried
    ...
    function _feature()
    {
        ...
        $state = Sanitize::getInt($this->data['Listing'],'featured');

        # Stop form data tampering 
        $formToken = cmsFramework::getCustomToken($listing_id,$state);
        ...
        if($this->Listing->feature($listing_id,!$state))
        {
    ...
    {
        $listing_id = Sanitize::getInt($this->data['Listing'],'id');
        $state = Sanitize::getInt($this->data['Listing'],'frontpage');
        ...
        if($this->Listing->frontpage($listing_id,$state))
        {
            return json_encode(array('error'=>false));        
        } 
    }
