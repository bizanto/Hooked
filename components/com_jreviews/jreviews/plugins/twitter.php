...
*/
    var $activities = array(
          'listing_new'=>"Listing: %1\$s. %2\$s", //#1 category title, #2 listing title
          'review_new'=>"Review for: %1\$s. %2\$s", //#1 listing title, #2 review title
          'comment_new'=>"Discussion on: %1\$s. %2\$s" //#1 listing title, #2 comment
     );    
    ...
    function _plgReviewAfterSave(&$model)
    {
        ...
        * Publish activity to Twitter
        */           
        if(isset($model->isNew) && $model->isNew && $review['Review']['published'] == 1 && $review['Review']['modified'] == NULL_DATE)
        {
    ...
    function sendTweet($message)
    {
        ...
        {
            App::import('Vendor','twitter' . DS . 'twitteroauth');
        }
