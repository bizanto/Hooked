...
class CommunityFriendsController extends CommunityBaseController
{
	...
	public function ajaxConfirmFriendRemoval($friendId)
	{
		...
		ob_start();
		?>
			<p><?php echo JText::sprintf('CC FRIEND REMOVAL WARNING', $user->getDisplayName()); ?></p>
			<br/>
			<input type="checkbox" name="block"/><?php echo JText::_('CC ALSO BLOCK FRIEND'); ?>
		<?php
		$html = ob_get_contents();
		ob_end_clean();
	...
	public function ajaxBlockFriend($id)
	{
		...
		if ($this->block($id))
		{
			$html = JText::sprintf('CC FRIEND BLOCKED', $user->getDisplayName());
			$objResponse->addScriptCall('joms.jQuery(\'#friend-' . $user->id . '\').remove();');
		} else {
			$html = JText::_('CC ERROR BLOCK USER');
		}
		...
		$objResponse->addAssign('cwin_logo', 'innerHTML', JText::_('CC BLOCK FRIEND'));
		$objResponse->addAssign('cWindowContent', 'innerHTML', $html);
		$objResponse->addScriptCall('cWindowActions', $actions);
		return $objResponse->sendResponse();
	}
	...
	public function ajaxRemoveFriend($id)
	{
		...
		if ($this->delete($id))
		{
			...
			$html = JText::sprintf('CC FRIEND REMOVED', $friend->getDisplayName());
			$objResponse->addScriptCall('joms.jQuery(\'#friend-' . $id . '\').remove();');
		} else {
			$html = JText::_('CC ERROR REMOVING FRIEND');
		}
		...
		$objResponse->addAssign('cwin_logo', 'innerHTML', JText::_('CC REMOVE FRIEND'));
		$objResponse->addAssign('cWindowContent', 'innerHTML', $html);
		$objResponse->addScriptCall('cWindowActions', $actions);
		return $objResponse->sendResponse();
	}
	...
	public function delete($id)
	{
		...
		$isFriend = CFriendsHelper::isConnected( $my->id, $friend->id );
		if (!$isFriend)
			return true;
