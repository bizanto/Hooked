...
class CommunityInboxController extends CommunityBaseController {
		...
		if ($saving = JRequest::getVar ( 'action', '', 'POST' ))
		{
			...
			if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
			{
				...
				if( $getBlockStatus->isUserBlocked($recepientId,'inbox') && !COwnerHelper::isCommunityAdmin() ){
					$view->addWarning ( JText::_('CC YOU ARE BLOCKED BY USER') );
					$validated = false;
					return;
		        }
				...
				if( $my->id == $validUser )
				{
					$mainframe->redirect(CRoute::_('index.php?option=com_community&view=inbox&task=write' , false) , JText::_('CC MESSAGE CANNOT SEND TO SELF') , 'error' );
					return;
				}
	...
	function ajaxAddReply($msgId, $reply) {
		...
		if( $getBlockStatus->isUserBlocked($messageRecepient[0],'inbox') && !COwnerHelper::isCommunityAdmin() ){
			$objResponse->addScriptCall( 'alert' , JText::_( 'CC YOU ARE BLOCKED BY USER' ) );
			$objResponse->sendResponse ();
			return;
        }
