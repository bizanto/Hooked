...
class CommunityProfileController extends CommunityBaseController
{
	...
	public function ajaxConfirmRemoveAvatar()
	{
		...
		$tmpl		= new CTemplate();
		$content	= JText::_('CC CONFIRM REMOVE PROFILE PICTURE');
		
		$response->addAssign('cwin_logo', 'innerHTML', JText::_('CC REMOVE PROFILE PICTURE') );
		...
		$action		= '<form action="' . $formAction . '" method="POST">';
		$action		.=	'<input class="button" type="submit" value="' . JText::_('CC BUTTON YES') . '" />';
	...
	public function removeAvatar()
	{
		...
		$mainframe->redirect( CRoute::_( 'index.php?option=com_community&view=profile' , false ) , JText::_('CC PROFILE PICTURE REMOVED') );
	}
	...
	public function ajaxPlayProfileVideo($videoid=null, $userid=0)
	{
		...
		if(!empty($video->id))
		{
			...
			else
			{
				...
				$content	=	$video->getPlayerHTML(null,null,false);
	...
	public function ajaxUpdateURL($userId)
	{
		...
		$objResponse->addAssign('cwin_logo', 'innerHTML', JText::_('CC PROFILE CHANGE ALIAS') );
		$objResponse->addAssign('cWindowContent', 'innerHTML', $content);

		$action		= '<input type="button" value="' . JText::_('CC BUTTON UPDATE') . '" class="button" onclick="joms.jQuery(\'#jsform-profile-ajaxupdateurl\').submit();" />';
		$action		.= '<input type="button" class="button" onclick="cWindowHide();return false;" value="'.JText::_('CC BUTTON CANCEL').'" />';
	...
	public function updateAlias()
	{
		...
		if( $userId != 0 )
		{
			...
			if( $this->_validVanityURL( $alias , $user->id ) )
			{
				...
				$user->save( 'params' );
				$message        = JText::_('CC PROFILE ALIAS UPDATED' );
			}
			else
			{
		...
		{			
			...
			if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
			{
                ...
                if($activity != 0)
                {
                    ...
                    {
                    	if( $this->_validVanityURL( $postvars['alias'] , $my->id ) )
						{
							$my->set( '_alias' , $postvars['alias'] );
						}
                        else
                        {
                        	$message        = JText::_('CC ALIAS ALREADY EXISTS' );
