...
<?php
	...
	public function ajaxShowLockDiscussion( $groupid, $topicid)
	{
		...
		$questionLock	= $discussion->lock ? JText::_('CC DISCUSSION UNLOCK MESSAGE') : JText::_('CC DISCUSSION LOCK MESSAGE');
...
<?php
		...
		$buttons	= '<form name="jsform-groups-ajaxshowlockdiscussion" method="post" action="' . CRoute::_('index.php?option=com_community&view=groups&task=lockTopic') . '">';
		$buttons	.= '<input type="submit" value="' . JText::_('CC BUTTON YES') . '" class="button" name="Submit"/>';
		$buttons	.= '<input type="hidden" value="' . $groupid . '" name="groupid" />';
		$buttons	.= '<input type="hidden" value="' . $topicid . '" name="topicid" />';
		$buttons	.= '&nbsp;';
	...
	public function ajaxConfirmMemberRemoval($memberId, $groupId)
	{
		...
		ob_start();
		?>
			<p><?php echo JText::sprintf('CC GROUP MEMBER REMOVAL WARNING', $member->getDisplayName()); ?></p>
			<br/>
			<input type="checkbox" name="block"/><?php echo JText::_('CC ALSO BAN MEMBER'); ?>
		<?php
		...
		$objResponse->addAssign('cwin_logo', 'innerHTML', JText::_('CC REMOVE MEMBER'));
...
<?php
	...
	public function ajaxUnbanMember( $memberId, $groupId )
	{
		...
		else
		{
			...
			$response->addScriptCall('joms.jQuery("#member_' . $memberId . '").css("border","3px solid green");');
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('CC GROUP MEMBER HAS BEEN UNBANNED') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","info");');
		}
	...
	public function ajaxBanMember( $memberId, $groupId )
	{
		...
		else
		{
			...
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('CC GROUP MEMBER HAS BEEN BANNED') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","info");');
		}
	...
	public function lockTopic()
	{
		...
		if( $my->id == $discussion->creator || $isGroupAdmin || COwnerHelper::isCommunityAdmin() )
		{
			...
			$confirmMsg	= $lockStatus ? JText::_('CC DISCUSSION LOCKED') : JText::_('CC DISCUSSION UNLOCKED');
		...
		{
			$mainframe->redirect( CRoute::_('index.php?option=com_community&view=groups&task=viewdiscussion&groupid=' . $groupid . '&topicid=' .$topicid , false ), JText::_('CC NOT ALLOWED TO LOCK GROUP TOPIC') );
		}
	}
	...
	public function sendmail()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( !$errors )
			{
				...
				$params->set( 'message' , $message );
				CNotificationLibrary::add( 'groups.sendmail' , $my->id , $emails , JText::sprintf( 'CC GROUPS SENDMAIL SUBJECT' , $group->name ) , '' , 'groups.sendmail' , $params );
				
				$mainframe->redirect( CRoute::_('index.php?option=com_community&view=groups&task=viewgroup&groupid=' . $group->id , false ) , JText::sprintf('CC EMAIL SENT TO GROUP MEMBERS' , $total ) );
			}
		}
