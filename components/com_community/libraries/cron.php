...
class CCron {
	...
	private function _processUserAvatarStorage( $updateNum = 5 )
	{
		$config		= CFactory::getConfig();
		$jconfig	= JFactory::getConfig();
		$storageMethod	= $config->getString( 'user_avatar_storage');

		echo JText::_('Moving user avatars to proper storage location...<br />');
		...
		if( !$rows )
		{
			echo JText::_('<br />No avatars needed to be transferred. <br />');
			return;
		}
		...
		foreach( $rows as $row )
		{
			...
			if( $current->exists($row->thumb) && $current->exists($row->avatar) )
			{
				...
				echo JText::sprintf('Downloading user thumbnail to temporary location: %1$s<br />' , $tmpThumbFileName );
				$current->get( $row->thumb , $tmpThumbFileName );
				
				$tmpAvatarFileName	= $jconfig->getValue( 'tmp_path' ) . DS . md5( $row->avatar );
				echo JText::sprintf('Downloading user avatar to temporary location: %1$s<br />' , $tmpAvatarFileName );
				...
				if( JFile::exists( $tmpThumbFileName ) && JFile::exists( $tmpAvatarFileName ) )
				{
					if( $storage->put( $row->avatar , $tmpAvatarFileName ) && $storage->put( $row->thumb , $tmpThumbFileName ) )
					{
						...
						$current->delete( $user->_avatar );
						$current->delete( $user->_thumb );
		...
		echo JText::sprintf( '%1$s file transferred...<br />' , $totalMoved );
	}
