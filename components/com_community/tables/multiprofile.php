...
class CTableMultiProfile extends JTable
{
	...
	public function getName()
	{
	    if( empty( $this->name ) )
	    {
	        return JText::_('CC DEFAULT PROFILE TYPE');
		}
	...
	{		
		$db		=& JFactory::getDBO();
		$query	= 'SELECT * FROM #__community_profiles_fields '
	...
	public function delete()
	{
		...
		$db		=& JFactory::getDBO();
		$query	= 'UPDATE #__community_users SET `profile_id`=' . $db->Quote( COMMUNITY_DEFAULT_PROFILE ) . ' '
				. 'WHERE `profile_id`=' . $db->Quote( $this->id );
		$db->setQuery( $query );
		$db->Query();
	...
	{
		$db		=& JFactory::getDBO();
		$query	= 'DELETE FROM #__community_profiles_fields WHERE ' . $db->nameQuote( 'parent' ) . '=' . $db->Quote( $this->id );
		$db->setQuery( $query );
		$db->Query();
	...
	public function updateUserDefaultImage( $type , $oldPath )
	{
		...
		$oldImagePath = JPATH_ROOT . DS . JString::str_ireplace( '/' , DS , $oldPath );
	...
	private function _updateUserWatermark( $user , $type , $hashName )
	{
		...
		$data			= @getimagesize( JPATH_ROOT . DS . JString::str_ireplace( '/' , DS , $user->$userImageType ) );
		...
		if( isset( $types[ $user->_profile_id ] ) )
		{
			...
			$watermarkPath	= JPATH_ROOT . DS . JString::str_ireplace('/' , DS , $this->watermark);
			...
			$oldFile	= JPATH_ROOT . DS . JString::str_ireplace( '/' , DS , $user->$userImageType );	
