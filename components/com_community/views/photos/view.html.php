...
class CommunityViewPhotos extends CommunityView
{
	...
	function _getAllAlbumsHTML( $albums , $type = PHOTOS_USER_TYPE, $pagination = NULL )
	{
		$my			= CFactory::getUser();
		$groupId	= JRequest::getInt( 'groupid' , '' ,'REQUEST');
		$handler	= $this->_getHandler();
	...
	function album()
	{
		...
		if (!empty($groupId))
		{
			...
			$pathway->addItem(JText::_('CC GROUPS'), CRoute::_('index.php?option=com_community&view=groups'));
			$pathway->addItem($group->name, CRoute::_('index.php?option=com_community&view=groups&task=viewgroup&groupid=' . $groupId));
			$pathway->addItem(JText::_('CC PHOTOS'), CRoute::_('index.php?option=com_community&view=photos&groupid=' . $groupId));
		}
	...
	function photo()
	{
		...
		if (!empty($groupId))
		{
			...
			$pathway->addItem(JText::_('CC PHOTOS'), CRoute::_('index.php?option=com_community&view=photos'));
		}
