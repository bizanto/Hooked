...
class CommunityModelPhotos extends JCCModel
{
	 ...
	 */	 
	function getAllPhotos( $albumId = null , $photoType = PHOTOS_USER_TYPE, $limit = null , $permission=null , $orderType = 'DESC' , $primaryOrdering = 'ordering')
	{
		$db		=& $this->getDBO();
...
// 				. 'LIMIT ' . $limitstart . ',' . $limit;
		...
		$query	= 'SELECT x.*, '
				...
				. ' a.`path` , a.`type` , a.`groupid`, a.`permanent`, ' /** HTGMOD **/
				. 'c.thumbnail, '
				. 'c.storage, '
				. 'c.id as photoid, '	
				. 'IF( a.groupid>0, IF( d.approvals=0,true,(' . $subQuery . ') ), true ) as display, '
...
// 				. 'or (a.permissions = 40 and a.creator = '. $db->Quote( $userId ) . ')) '
	...
	public function getPopularPhotos( $limit = 20 , $permission = null )
	{
	    ...
	    $query  = 'SELECT `id` FROM #__community_photos '
				. 'WHERE ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( 1 );
