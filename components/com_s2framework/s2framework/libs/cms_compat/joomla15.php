...
// Reverted change to getDocument class because it causes problems in the administration. listing submit                
//                $doc =& JFactory::getDocument();
//                $doc->addCustomTag($text);
            }
    ...
    */
    ...
    function formIntegrityToken($entry, $keys, $input = true)
    {
        ...
        foreach($keys AS $key)
        {
            if(isset($entry[$key])) $string .= is_array($entry[$key]) ? serialize($entry[$key]) : $entry[$key];
        }
