 ...
 */
	...
	# remove unused keys
	var $__passedVars = array('app','assets','viewVars', 'view', 'Access', 'action', 'autoLayout', 'autoRender', 'base', 'cacheAction', 'Config', 'webroot', 'helpers', 'here', 'limit', 'layout', 'modelNames', 'module_limit', 'module_page', 'name', 'offset', 'page', 'pageTitle', 'viewSuffix', 'viewTheme', 'viewPath', 'params', 'data', 'webservices', 'plugin', 'passedArgs', 'rawData', 'xajaxRequest', 'ajaxRequest');

	# These are passed to the Helper Object for use in helpers
	var $__helperVars = array('Access','action','app','assets','name','data','page','params','passedArgs','Config','limit','module_limit','module_page', 'viewSuffix', 'viewTheme','viewImages','viewImagesPath','xajaxRequest','ajaxRequest');
...
/*		if (!empty($this->loaded)) {
    ...
    /**
    * Used for handling errors in theme files- Alejandro - added May 6th 2009
    */
    function ViewErrorHandler($errno, $errstr, $errfile, $errline)
    {
	...
	function _render($___viewFn, $dataForView, $loadHelpers = true, $cached = false) {
		...
		if ($loadHelpers && !(empty($this->helpers))) {
			...
			{								
				...
				if (!isset($this->loaded[$method_name])) {
                    ...
                    if(in_array($class_name,array('RoutesHelper','PaginatorHelper','JreviewsHelper')))  /*creates problem with $this->requestAction method - pagination urls are affected for component */   
                        ${$method_name} = new $class_name($this->app); 
                    else 
                        ${$method_name} = & RegisterClass::getInstance($class_name,$this->app); 
        ...
        // Set view error handling - Alejandro - added May 6th 2009 
        //set_error_handler(array(&$this,'ViewErrorHandler'),error_reporting(E_ALL));
        ...
        // Restore error handling - Alejandro - added May 6th 2009
...
function &_loadHelpers(&$loaded, $helpers, $parent = null) {
	...
	{								
		...
		if (!isset($this->loaded[$method_name])) {
			...
			${$method_name} = RegisterClass::getInstance($class_name);
