 ...
 */
   ...
   function __listFiles($basePath,$relPath,$recursive,$type) {
        ...
        if (is_array($contents)) {
            ...
            foreach ($contents[1] as $item) {
                if($recursive){
                    $this->__items[$type][$relPath][strtolower($item)] = str_replace(PATH_ROOT,'',rtrim($relPath,DS)).DS.$item;                    
                } else {
                    $this->__items[$type][strtolower($item)] = str_replace(PATH_ROOT,'',rtrim($basePath.$relPath,DS)).DS.$item;
                }
            }
        }
    }    
                    ...
                    if(is_array($tree))
                    {
                        foreach($tree[1] AS $file)
                        {
                            ...
                            if(count($path)>2) 
                            {
                                ...
                                array_pop($parts);        
                                if($secondToLast == 'theme_css' || $thirdToLast == 'theme_css') array_pop($parts);                                 
                                if($thirdToLast == 'theme_css') array_pop($parts);                                 
                                ...
                                if($theme_subdir == 'theme_css' && !strstr($theme_file,'.css')) continue;

                                $items['Theme'][$theme_folder][$theme_subdir][$theme_file] = $theme_file;  // $theme_file_path;
                                ...
                                if(
                                        $items['Theme'][$theme_folder]['.info']['location'] != "jreviews" // Not in component folder
                                        &&  
                                        !isset($items['Theme']['default'][$theme_subdir][$theme_file]) // Not in default folder
                                ) {         
                    ...
                    if(is_array($tree))
                    {
                        foreach($tree[1] AS $file)
                        {
                            ...
                            if(count($path)>2) {
                                ...
                                $items['AdminTheme'][$theme_folder][$theme_subdir][$theme_file] = $theme_file; //$theme_file_path;                              
                                if(
