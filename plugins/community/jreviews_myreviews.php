...
class plgCommunityJreviews_myreviews extends CApplications
{
    ...
    function plgCommunityJreviews_myreviews(& $subject, $config)
    {
    ...
    {   
        if( !file_exists( $this->_path . DS . 'admin.jreviews.php' ) ){
            ...
            if($cacheSetting) {
                ...
                $Html->css(array('theme','plugins','paginator'), $Libraries->css());
                $Html->js(array('jreviews','jquery'=>'jquery','jq.scrollable'), $Libraries->js());
            }

            $cache =& JFactory::getCache('plgCommunityJreviews_myreviews');
            $cache->setCaching($cacheSetting);
            $callback = array('plgCommunityJreviews_myreviews', '_getPage');
