			...
			<table class="adminlist" cellspacing="1">
				<thead>
					<tr>
						...
						<?php if ( $client->id != 1 && $config->show_tooltip != 1 ) : ?>
				...
				for ( $i=0, $n=count( $rows ); $i < $n; $i++ ) {
					...
					if ( $config->show_tooltip != 2 && isset( $advanced_params->tooltip ) && $advanced_params->tooltip ) {
		...
		<div class="col width-50">
			<fieldset class="adminform">
				...
				?>
					<tr>
						<td valign="top" class="key">
							<label for="hideempty" class="hasTip" title="<?php echo JText::_( 'AMM_HIDE_IF_EMPTY' ); ?>::<?php echo JText::_( 'AMM_HIDES_THE_MODULE_IF_ITS_OUTPUT_IS_EMPTY' ); ?>">
	...
	*/
	...
	function renderAssignments( &$lists )
	{
		...
		if (	$config->show_match_method
			&& (	$config->show_assignto_secscats
				||	$config->show_assignto_articles
				||	$config->show_assignto_k2cats
				||	$config->show_assignto_mrcats
				||	$config->show_assignto_zoocats
		...
		if (	$config->show_assignto_secscats
			||	$config->show_assignto_articles
			||	( $config->show_assignto_k2cats && JFile::exists( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_k2'.DS.'admin.k2.php' ) )
			||	( $config->show_assignto_mrcats && JFile::exists( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_resource'.DS.'resource.php' ) )
			||	( $config->show_assignto_zoocats && JFile::exists( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_zoo'.DS.'zoo.php' ) )
		) {
