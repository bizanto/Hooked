...
class CommunityControllerProfiles extends CommunityController
{
	...
	function ajaxSaveGroup( $id , $data )
	{
		...
		if( $isValid )
		{
			...
			if($id != 0)
			{
				$name	= '<a href="javascript:void(0);" onclick="azcommunity.editFieldGroup(\'' . $row->id . '\' , \'' . JText::_('CC EDIT GROUP') . '\');">' . $row->name . '</a>';
	...
	function ajaxGetFieldParams( $type )
	{
	...
	function ajaxEditGroup( $fieldId , $isGroup = false )
	{
		...
		$windowTitle    = ($row->id == 0 ) ? JText::_('CC NEW GROUP') : JText::_('CC EDIT GROUP');
		ob_start();
?>
