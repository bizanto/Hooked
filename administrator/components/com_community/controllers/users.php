		...
		else
		{
			if( $row->$field == 1 )
			{
				...
				if( $row->lastvisitDate == '0000-00-00 00:00:00' && empty($row->activation) )
				{
					...
					$subject 	= JText::sprintf( 'CC ACCOUNT IS APPROVED SUBJECT' , $name, $sitename);
					$subject 	= html_entity_decode($subject, ENT_QUOTES);
		
					$message	= sprintf ( JText::_( 'CC ACCOUNT IS APPROVED' ), $siteURL , $row->name , $row->email , $row->username );
