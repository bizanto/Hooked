...
//					$index = isset($index{0}) ? $index{0} : '';
	...
	function search() 
    {
		...
		$related = Sanitize::getString($this->params,'related');
		...
		if ($related) {
			$related = str_replace($urlSeparator, ",", $related);
			$wheres[] = "Listing.id IN (SELECT id2 FROM #__relate_listings WHERE id1 IN ($related))";
		}
				...
				if (substr($key,0,3)=="jr_" && in_array($key,$customFields) && !is_null($value) && $value != '') { // Check if the field exists
					...
					if (substr_count($value,$urlSeparator)) {
						...
						// This is a field with multiple options
							...
							# HTGMOD #
							$OR_fields = array("select","radiobuttons","selectmultiple","checkboxes");
							$AND_fields = array("");
