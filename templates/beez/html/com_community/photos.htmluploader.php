...
?>
	...
	<?php if ($relate_id) {
		$db =& JFactory::getDBO();
		$sql = "SELECT id, title, catid FROM #__content WHERE id = $relate_id";
		$db->setQuery($sql);
		$listing = $db->loadObject();	
		$rel_href = CRoute::_('index.php?option=com_content&view=article&catid='.$listing->catid.'&id='.$listing->id);
		$rel_link = '<a href="'.$rel_href.'">'.$listing->title.'</a>';
	?>
	<br/>
	<div>
	<?php echo JText::_('CC BUTTON GO BACK').': '.$rel_link; ?>
	</div>
	<?php } ?>
	<br/>
	...
	if( !empty( $albumId ) && $albumId != -1 )
	{
		if ($relate_id) $relid = "&relate_id=$relate_id";
		else $relid = '';
	?>
	<script type="text/javascript" language="javascript">
	joms.uploader.postUrl 		= '<?php echo CRoute::_('index.php?option=com_community&view=photos&task=jsonupload&no_html=1&tmpl=component&defaultphoto=DEFAULT_PHOTOS&nextupload=NXUP&albumid=' . $albumId . $relid, false );?>';
